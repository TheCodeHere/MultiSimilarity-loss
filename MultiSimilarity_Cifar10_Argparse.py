...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channels=3, output_size=2, alpha=2, beta=20, base=1.0):
...
def ResNet_size(size=10, in_channels=3, output_size=2, alpha=2, beta=50, base=1.0):
    if size == 10:
...
# Model (on Device)
Network_model = to_device(ResNet_size(size=18, in_channels=3, output_size=output_dim, alpha=alpha, beta=beta, base=base), device)
...
File_name = "Log/MultiSim/Cifar10/" + str(output_dim) + "d/Cifar_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_a-" + str(alpha) + "_bt" + str(beta) + "_b-" + str(base) + ".pth"
