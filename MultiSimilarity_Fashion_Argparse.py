...
parser.add_argument('-a','--alpha', type=float, metavar='', required=True, help='The weight applied to positive pairs')
parser.add_argument('-bt','--beta', type=float, metavar='', required=True, help='The weight applied to negative pairs')
parser.add_argument('-b','--base', type=float, metavar='', required=True, help='offset applied to the exponent in the loss')
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channels=3, output_size=2, alpha=1, beta=40, base=1.0):
        ...
        self.MultiSim_loss = losses.MultiSimilarityLoss(alpha=alpha, beta=beta, base=base, reducer=reducers.MeanReducer())
        self.MultiSim_loss_without_reduction = losses.MultiSimilarityLoss(alpha=alpha, beta=beta, base=base, reducer=reducers.DoNothingReducer())
    ...
    def training_step(self, batch):
        ...
        # Calculate loss
        loss = self.MultiSim_loss(embeddings, labels)
...
def ResNet_size(size=10, in_channels=3, output_size=2, alpha=1, beta=40, base=1.0):
    if size == 10:
        return ResNet(BasicBlock, [1, 1, 1, 1], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
    elif size == 18:
        return ResNet(BasicBlock, [2, 2, 2, 2], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
    elif size == 34:
        return ResNet(BasicBlock, [3, 4, 6, 3], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
    elif size == 50:
        return ResNet(Bottleneck, [3, 4, 6, 3], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
    elif size == 101:
        return ResNet(Bottleneck, [3, 4, 23, 3], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
    elif size == 152:
        return ResNet(Bottleneck, [3, 8, 36, 3], in_channels=in_channels, output_size=output_size, alpha=alpha, beta=beta, base=base)
...
# Model (on Device)
Network_model = to_device(ResNet_size(size=18, in_channels=1, output_size=output_dim, alpha=alpha, beta=beta, base=base), device)
...
File_name = "Log/MultiSim/Fashion/" + str(output_dim) + "d/Fashion_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_a-" + str(alpha) + "_bt" + str(beta) + "_b-" + str(base) + ".pth"
