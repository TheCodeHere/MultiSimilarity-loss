...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channels=3, output_size=2, alpha=2, beta=50, base=1.0):
...
# Model (on Device)
Network_model = to_device(ResNet_size(size=18, in_channels=1, output_size=output_dim, base=base), device)
...
File_name = "Log/MultiSim/Fashion/" + str(output_dim) + "d/Fashion_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_b-" + str(base) + ".pth"
